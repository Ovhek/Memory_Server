   ...
   public void destroy() {
    ...
    public void addArticle(Article a) throws CompraException {
        ...
        if (this.idSessioCompra != null)
        {
            ...
            this.compra.incrementaContador();
            
            l.setOrdinal(this.compra.getContador());
    ...
    @Override
    public List<LiniaCompra> getCurrentLines() {
        
        return this.compra.getLinies();
        
    }

    @Override
    public void deleteCurrentLine(Integer _ordinal) throws CompraException {
        
        log.log(Level.INFO, "Eliminant linia de compra: {0}", _ordinal);
        
        List<LiniaCompra> newLinies = compra.getLinies().stream().filter(x-> !x.getOrdinal().equals(_ordinal)).collect(Collectors.toList());
        
        this.compra.setLinies(newLinies);
    }
}